...
b. 修改TRANSACTION_AUDIT_ID对应buyer_id的waf_wallet_account_fund 钱包余额表 。可用金额=可用金额+充值中金额-提现金额。
...
2、管理员看到全部的订单和商品单独写接口还是改现在的  √改现有的
...
| 字段         | 类型       | 含义                                | 值     |
| ------------ | ---------- | ----------------------------------- | ------ |
| BUYER_ID     | int(10)    | 主键                                | userid |
| IS_ACTIVE    | char(1)    | 是否激活 N - 未激活 , Y -  激活     | N      |
| STATUS       | tinyint(2) | 状态 7 -正常,   17 -冻结            | 7      |
| ACCOUNT_TYPE | int(11)    | 账户类型:0-管理员 1-品牌商 2-借卖方 | roleid |

6、GMC表中buyerid改为string=userid